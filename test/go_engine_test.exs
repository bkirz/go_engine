defmodule GoEngineTest do
  ...
  test "rejects placing stones out of bounds" do
    ...
    assert {:invalid, :out_of_bounds} =
             GoEngine.play_move(state, {:black, {:place_stone, {0, 0}}})

    assert {:invalid, :out_of_bounds} =
             GoEngine.play_move(state, {:black, {:place_stone, {0, 1}}})

    assert {:invalid, :out_of_bounds} =
             GoEngine.play_move(state, {:black, {:place_stone, {1, 0}}})

    assert {:invalid, :out_of_bounds} =
             GoEngine.play_move(state, {:black, {:place_stone, {6, 6}}})

    assert {:invalid, :out_of_bounds} =
             GoEngine.play_move(state, {:black, {:place_stone, {1, 6}}})

    assert {:invalid, :out_of_bounds} =
             GoEngine.play_move(state, {:black, {:place_stone, {6, 1}}})
  end
  ...
  test "rejects placing on existing stones" do
    ...
    assert {:invalid, :stone_present} =
             GoEngine.play_move(state, {:white, {:place_stone, {1, 1}}})
  end
